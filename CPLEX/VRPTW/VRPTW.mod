...
subject to{
	...
	//Constrant 3 - makes sure a node entered once - WORKS
	forall(j in 2..nLocations){
		sum(k in K, i in I) x_kij[k][i][j] == 1;
		
	}
	
	//Constraint 4 - make sure a node is only exited once
	forall(i in 2..nLocations){
		sum(k in K, j in J) x_kij[k][i][j] == 1;
	}
	
	forall(k in K){
		sum(i in 2..nLocations, j in 2..nLocations)x_kij[k][i][j] ==1;  
 	}	  
	
	//Constraint 5 - a vehicle leaves the node it entered - this also fixes the problem for teleporting??
	forall(k in K){
		forall(i in 2..nLocations){
		  sum(j in 2..nLocations)x_kij[k][j][i] - sum(j in 2..nLocations)x_kij[k][i][j] == 0;
  		}		  
 	}
 	
 	//Constraint to evade sub tours	  

	
	
	//Constraint - make sure a vehicle starts in node 1
	forall(k in K){
		sum(i in 2..nLocations)x_kij[k][1][i] <= 1;  
 	}	  
